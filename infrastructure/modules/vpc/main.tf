# ── modules/vpc ──────────────────────────────────────────────────────────────
# Required resources (Task B1): aws_vpc, aws_subnet (public only), aws_internet_gateway,
# aws_route_table, aws_route_table_association, aws_security_group.
#
# Mirrors the manual Part A build: one public subnet routed to the internet
# via an IGW, and a security group that only allows traffic from inside the VPC.
